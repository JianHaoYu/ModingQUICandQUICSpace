...
def analysis(loss_rate, repair_rate, total_packets=20, delay=500):
    ...
    Analysis_total_time = Analysis_arrival_time + rho/(1-rho)
...
def run(loss_rate, repair_rate, random_seed, total_packets=200, delay=500):
    ...
    while True:
        ...
        # Send stage
        if LastSendSourceID < total_packets and random.random() >= repair_rate:
            LastSendSourceID += 1
...
def runsmple(loss_rate = 0.1,repair_rate = 0.16,total_packets = 200 ,delay = 2000 ):
    # Parameters
    trials = 20000
